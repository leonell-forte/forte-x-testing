...
import type { IFilters, OrgTypes } from "@/lib/types/organizations";
          ...
          <div className="flex gap-2.5 md:flex-wrap">
            <div className="w-full md:w-auto">
              <SearchInput
                ...
                tooltip={`Search by ${type} name, registered name, or registration number.`}
              />
            </div>
            <button
              onClick={(e) => {
...
const Filters = ({ filters, setFilters, initialFilters }: IFilterProps) => {
  ...
  return (
    <div className="grid w-full grid-cols-1 gap-2.5 lg:flex">
      ...
      >
        <FilterIcon
          className="group-hover:fill-mint group-hover:stroke-mint h-auto w-5 fill-white transition-all"
          aria-hidden="true"
          role="presentation"
        />
      </button>
