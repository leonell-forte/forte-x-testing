...
import type { LoginReturnType } from "../types/users";

import type { LoginCopy, UserData } from "../types/auth";
import type { LoginFormData } from "@/features/auth/form";
...
class AuthService {
  ...
  async login(body: LoginFormData): Promise<LoginReturnType> {
  ...
  // update types later
  async signup(body: any, code: string = "") {
  ...
  // update types later
  async resetPassword(values: any, email: string) {
