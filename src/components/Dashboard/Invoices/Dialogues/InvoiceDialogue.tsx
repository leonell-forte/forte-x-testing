...
interface IInvoiceDialogue extends IDialogueProps {
  id?: string;
}

const InvoiceDialogue = ({ id, ...props }: IInvoiceDialogue) => {
    ...
    >
      {isLoading ? (
        ...
        <div className="space-y-10">
          ...
          <div className="table-breakpoint:hidden">
            <Cards.Container className="!grid-cols-1" isLoading={isLoading}>
              {data?.items.map((item, index) => {
                ...
                return (
                  <Cards.Card key={index} title={milestone.title}>
                    <Cards.Group cols={2}>
          ...
          <div className="hidden w-full overflow-x-auto table-breakpoint:block">
            <Table.Container>
              <Table.Head>
                <Table.Row>
